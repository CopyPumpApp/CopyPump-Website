...
const splitSummary=(text:string,locale:'en'|'ru')=>{
...
function SummaryBlock({text,compact=false}:{text:string;compact?:boolean}){
  const {locale}=useI18n(),c=radarCopy[locale],parts=splitSummary(text,locale)
  return <div className={compact?'radar-summary radar-summary--compact':'radar-summary'}>
    <p className="radar-summary__label">{c.short}</p>
    <p className="radar-summary__text">{parts.short}</p>
    {parts.why&&<p className="radar-summary__why"><strong>{c.whyShort}</strong> {parts.why}</p>}
  </div>
}
...
const sourceContext=(item:ObservationSummary|Observation,locale:'en'|'ru')=>item.sourceKind==='web'?(item.sourceLabel?.[locale]||(locale==='ru'?'WEB / ПРОВЕРЕННЫЕ ИСТОЧНИКИ':'WEB / VERIFIED SOURCES')):(item.cluster==='devnet'?'SOLANA DEVNET':'SOLANA MAINNET')
...
function RadarArticle({id}:{id:string}){
  ...
  return <div className="app-shell premium-v50 radar-page"><Seo title={`${entry.title[locale]} — CopyPump Radar`} description={entry.summary[locale]} path={`/radar/${id}`}/><main id="main-content" tabIndex={-1}>
    <header className="document-hero wrap radar-article-hero" data-scene="document"><Link to="/radar" className="text-link">← {c.back}</Link><p className="eyebrow">{entry.category[locale]} · {sourceContext(entry,locale)}</p><KineticHeading as="h1" lines={[{text:entry.title[locale],accent:true}]}/><SummaryBlock text={entry.summary[locale]}/><div className="radar-article-actions"><Save id={id}/>{changed&&<Link to={`/radar/${id}#editorial-history`} className="text-link">{c.show} ↓</Link>}</div><LocalNotice/>{mismatch&&<p role="status" className="radar-notice">{c.mismatch}<button type="button" className="text-link" onClick={()=>{s.reload();setAttempt(x=>x+1)}}>{c.retry} ↻</button></p>}</header>
    <article className="radar-article wrap"><aside className="radar-provenance"><p className="eyebrow">{entry.sourceKind==='web'?(locale==='ru'?'ИСТОЧНИКИ':'SOURCES'):c.network}</p><dl>{([[c.event,entry.observedAt],[c.published,entry.publishedAt],[c.checked,entry.lastCheckedAt]]).map(([k,v])=><div key={k}><dt>{k}</dt><dd><Stamp value={v}/></dd></div>)}</dl>{entry.sourceKind!=='web'&&entry.signature&&<p className="radar-signature">{c.signature}<code>{entry.signature}</code></p>}</aside>
