...
export function RadarTeaser(){
  const {locale}=useI18n(),c=radarCopy[locale],s=useRadar(),item=s.index?.items[0]
  return <section className="radar-teaser wrap section-space" id="radar-preview" data-scene="radar" aria-labelledby="radar-teaser-title">
    <div><p className="eyebrow">02 / COPY PUMP RADAR</p><KineticHeading id="radar-teaser-title" lines={[{text:c.teaserTitle},{text:c.teaserAccent,accent:true}]}/><p className="radar-teaser-note">{c.edition}</p></div>
    <div className="radar-teaser-story">{s.error&&<p className="radar-notice" role="status">{c.error} {item&&c.last}</p>}
