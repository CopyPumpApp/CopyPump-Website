...
import {useClientReady} from '../site/useClientReady'
import {publicationCopy, PUBLIC_EVIDENCE} from './publication'
...
function LocaleSwitch(){const {locale,setLocale}=useI18n(),ready=useClientReady();return <div className="locale-switch" role="group" aria-label={locale==='ru'?'Язык':'Language'}>{(['en','ru'] as const).map(l=><button type="button" key={l} disabled={!ready} aria-pressed={locale===l} onClick={()=>setLocale(l)}>{l.toUpperCase()}</button>)}</div>}
export function PremiumMotionToggle(){const m=useMotion(),c=useCopy().nav,ready=useClientReady();return <button type="button" className="motion-toggle" onClick={m.toggle} aria-pressed={!m.paused&&!m.reduced} disabled={!ready||m.reduced} aria-label={m.reduced?c.reduced:m.paused?c.play:c.pause}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 12h3l3-7 5 14 3-7h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg><span>{c.motion}</span><i/></button>}

export function PremiumHeader({simple=false,hideChannels=false}:{simple?:boolean;hideChannels?:boolean}) {
  const c=useCopy(), menu=useAnimatedMenu(), {locale}=useI18n(),ready=useClientReady()
  ...
  return <><span className="header-sentinel" ref={sentinel} aria-hidden="true"/><header className="site-header" ref={header}><a className="skip-link" href="#main-content">{c.nav.skip}</a>
    <div className="site-header__inner wrap"><Mark/>
      ...
      <div className="header-actions"><LocaleSwitch/><PremiumMotionToggle/>
        <button ref={menu.trigger} type="button" className="menu-button" disabled={!ready} aria-label={c.nav.menu} aria-expanded={menu.mounted} aria-haspopup="dialog" aria-controls="mobile-navigation" onClick={menu.open}><span/><span/></button>
      </div>
    </div>
  </header>
...
function Layout({children,title,description,path}:{children:ReactNode;title:string;description:string;path:string}){return <div className="app-shell premium-v51" data-release="52.0-content"><Seo title={title} description={description} path={path}/><main id="main-content" tabIndex={-1}>{children}</main></div>}
function DateLabel(){const {locale}=useI18n();return <time dateTime={STATUS.sourceDate}>{publicDate(STATUS.sourceDate,locale)}</time>}
function StatusBadge(){const c=useCopy();return <Link to="/progress" className="status-badge"><i/>{c.hero.label}<Arrow diagonal/></Link>}

function ControlSummary(){const {locale}=useI18n(),c=publicationCopy[locale].control;return <section className="control-summary wrap section-space" id="your-control" data-scene="control" aria-labelledby="control-title"><div className="section-heading"><div><p className="eyebrow">{c.eyebrow}</p><KineticHeading id="control-title" lines={[{text:c.title},{text:c.accent,accent:true}]}/></div><p>{c.intro}</p></div><div className="responsibility-columns">{[[c.system,c.systemItems],[c.user,c.userItems]].map(([title,items],i)=><div key={i}><h3>{title as string}</h3><dl>{(items as string[][]).map(([name,text])=><div key={name}><dt>{name}</dt><dd>{text}</dd></div>)}</dl></div>)}</div><div className="control-boundary"><p>{c.ai}</p><Link to="/project" className="text-link">{c.detail}<Arrow/></Link></div></section>}
function EvidenceLinks(){const {locale}=useI18n(),c=publicationCopy[locale].evidence;return <div className="evidence-links"><header><p className="eyebrow">{c.available}</p><h3>{c.title}</h3><p>{c.copy}</p></header><div className="evidence-grid"><article><h4><External href={PUBLIC_EVIDENCE.tools}>{c.toolsTitle}<Arrow diagonal/></External></h4><p>{c.toolsCopy}</p><p className="evidence-limit">{c.toolLimit}</p></article><article><h4><External href={PUBLIC_EVIDENCE.architecture}>{c.architectureTitle}<Arrow diagonal/></External></h4><p>{c.architectureCopy}</p><p className="evidence-next"><strong>{c.next}</strong>{c.nextCopy}</p></article></div><External href={PUBLIC_EVIDENCE.status} className="text-link">{c.source}<Arrow diagonal/></External></div>}
function ProductOverview(){const {locale}=useI18n(),c=publicationCopy[locale].projectOverview;return <section className="product-overview wrap" aria-labelledby="architecture-title"><p className="eyebrow">{c.eyebrow}</p><KineticHeading id="architecture-title" lines={[{text:c.title,accent:true}]}/><p className="overview-intro">{c.copy}</p><div className="architecture-parts">{c.parts.map(([title,text],i)=><article key={title}><span aria-hidden="true">0{i+1}</span><h3>{title}</h3><p>{text}</p></article>)}</div><div className="ai-boundary"><h3>{c.aiTitle}</h3><p>{c.aiCopy}</p><External href={PUBLIC_EVIDENCE.architecture} className="text-link">{c.source}<Arrow diagonal/></External></div></section>}

function ProductExperience(){
  const {locale}=useI18n(), c=useCopy().experience,m=useMotion(),ready=useClientReady()
  ...
  const chapter=c.chapters[active],allowed=limit>=.5,details=publicationCopy[locale].chapterDetails[active]
  return <section className="experience wrap section-space" id="experience" data-scene="experience" aria-labelledby="experience-title">
    ...
    <div className="experience-stage" ref={root} id="experience-panel" aria-busy={requested!==active} role="tabpanel" aria-labelledby={`chapter-${active}`} tabIndex={0} data-ambient={m.running&&inView?'on':'off'}>
      <div className="experience-art" data-reveal="depth" {...swipe} role="group" aria-label={locale==='ru'?'Переключение сцен продукта':'Product scene controls'} aria-describedby="artwork-swipe-hint" tabIndex={0} onKeyDown={event=>{if(event.target!==event.currentTarget)return;if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();stepChapter(event.key==='ArrowRight'?1:-1)}}}>
        ...
        <div className="artwork-step-controls"><button type="button" className="artwork-previous" disabled={!ready||requested===0} aria-label={locale==='ru'?'Предыдущий объект':'Previous object'} onClick={()=>stepChapter(-1)}>‹</button><span id="artwork-swipe-hint">{locale==='ru'?'Листайте объекты свайпом':'Swipe to explore'}</span><button type="button" className="artwork-next" disabled={!ready||requested===ART.objects.length-1} aria-label={locale==='ru'?'Следующий объект':'Next object'} onClick={()=>stepChapter(1)}>›</button></div>
      </div>
      <div className="experience-story chapter-enter" key={active}>
        <KineticHeading as="h3" lines={[{text:chapter.title,accent:true}]}/><p>{chapter.copy}</p><dl className="chapter-explanations">{details.map(([title,copy])=><div key={title}><dt>{title}</dt><dd>{copy}</dd></div>)}</dl>
        {active===2&&<div className="instrument instrument--policy"><div className="policy-readout"><span>{c.test}</span><b>0.50 <small>SOL</small></b></div><label className="policy-range" htmlFor="capital-limit"><span>{c.limit}</span><output htmlFor="capital-limit">{limit.toFixed(2)} SOL</output><input id="capital-limit" disabled={!ready} type="range" min="0.1" max="1" step="0.05" value={limit} onChange={e=>setLimit(Number(e.target.value))}/><small>0.10 SOL</small><small>1.00 SOL</small></label><div className={`policy-result ${allowed?'allowed':'blocked'}`} role="status" aria-live="polite"><strong><i/>{allowed?c.allowed:c.blocked}</strong><p>{allowed?c.allowedNote:c.blockedNote}</p></div></div>}
...
export function PremiumLanding(){const c=useCopy(),{locale}=useI18n(),p=publicationCopy[locale];return <Layout title={`CopyPump — ${c.hero.line1} ${c.hero.line2}`} description={c.hero.intro} path="/">
  <section className="hero" data-scene="hero" aria-labelledby="hero-title"><div className="hero-content wrap"><div className="hero-topline"><p className="eyebrow" data-reveal="label">{c.hero.eyebrow}</p><StatusBadge/></div><div className="hero-type"><KineticHeading as="h1" id="hero-title" lines={[{text:c.hero.line1},{text:c.hero.line2,accent:true}]}/><p className="hero-intro">{c.hero.intro}</p><p className="hero-context">{p.heroNote}</p><div className="hero-actions" data-reveal data-delay="210"><Link to="/#experience" className="button button-primary">{c.hero.primary}<Arrow/></Link><Link to="/progress" className="text-link">{c.hero.secondary}<Arrow diagonal/></Link></div></div><ul className="hero-benefits">{p.benefits.map(text=><li key={text}>{text}</li>)}</ul></div></section>
  <ProductExperience/>
  <ControlSummary/>
  <section className="status-section wrap section-space" id="status" data-scene="status" aria-labelledby="status-title"><div className="status-copy"><p className="eyebrow" data-reveal="label">{c.status.eyebrow}</p><KineticHeading id="status-title" lines={[{text:c.status.title},{text:c.status.accent,accent:true}]}/><p data-reveal>{c.status.intro}</p><Link to="/progress" className="text-link">{c.status.link}<Arrow/></Link></div><div className="status-card"><h3 data-reveal>{c.status.now}</h3><dl><div data-reveal data-delay="70"><dt>{c.status.network}</dt><dd>Solana Devnet</dd></div><div data-reveal data-delay="140"><dt>{c.status.mainnet}</dt><dd className="locked-label">{c.status.locked}</dd></div></dl><p className="status-source" data-reveal data-delay="210">{c.status.source} <DateLabel/></p><p className="fine-print">{c.status.note}</p></div><EvidenceLinks/></section>
  <RadarTeaser/>
  <section className="community wrap section-space" id="community" data-scene="community" aria-labelledby="community-title"><div><p className="eyebrow" data-reveal="label">{c.cta.eyebrow}</p><KineticHeading id="community-title" lines={[{text:c.cta.title,accent:true}]}/><p data-reveal>{c.cta.copy}</p><div className="community-actions" data-reveal data-delay="140"><External href={CHANNELS.discord} className="button button-primary">{c.cta.community}<Arrow diagonal/></External><External href={PUBLIC_EVIDENCE.issues} className="text-link">{p.community.issues}<Arrow diagonal/></External></div><p className="contributor-start">{p.community.contributor}</p><External href={CHANNELS.email} className="partner-link">{c.cta.partners} ↗</External></div></section>
  </Layout>}

export function PremiumProject(){const c=useCopy(),ready=useClientReady(),[tab,setTab]=useState(0),tabs=useRef<(HTMLButtonElement|null)[]>([]);useLayoutEffect(()=>{window.dispatchEvent(new Event('copypump:content-ready'))},[tab]);return <Layout title={`${c.nav.product} — CopyPump`} description={c.product.intro} path="/project"><section className="document-hero wrap" data-scene="document"><p className="eyebrow">{c.product.eyebrow}</p><KineticHeading as="h1" lines={[{text:c.product.title},{text:c.product.accent,accent:true}]}/><p>{c.product.intro}</p></section><ProductOverview/><section className="product-document wrap" aria-label={c.nav.product}><div className="document-tabs" role="tablist" aria-label={c.nav.product}>{c.product.tabs.map((label,i)=><button key={label} ref={el=>{tabs.current[i]=el}} type="button" disabled={!ready} role="tab" id={`product-tab-${i}`} aria-selected={tab===i} aria-controls="product-panel" tabIndex={tab===i?0:-1} onClick={()=>setTab(i)} onKeyDown={e=>{let n=i;if(e.key==='ArrowRight')n=(i+1)%3;else if(e.key==='ArrowLeft')n=(i+2)%3;else if(e.key==='Home')n=0;else if(e.key==='End')n=2;else return;e.preventDefault();setTab(n);tabs.current[n]?.focus()}}>{label}</button>)}</div><div role="tabpanel" id="product-panel" aria-labelledby={`product-tab-${tab}`} tabIndex={0} className="document-panel"><div className="document-tab-content" key={tab}>
