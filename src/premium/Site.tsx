...
export function PremiumLanding(){const c=useCopy(),{locale}=useI18n();const proofTarget=locale==='ru'?{label:'ПОСЛЕДНЕЕ ПОДТВЕРЖДЁННОЕ ОБНОВЛЕНИЕ',title:c.progress.currentTitle,copy:c.progress.currentCopy}:{label:'LATEST VERIFIED UPDATE',title:c.progress.currentTitle,copy:c.progress.currentCopy};return <Layout title="CopyPump — Smart money. Your rules." description={c.hero.intro} path="/">
