...
export function HeroScene() {
  ...
  useEffect(() => {
    ...
    const preload = workflowObjects.map(name => new Promise<void>(resolve => {
      ...
      image.src = `/workflow-objects/${name}.webp`
      const done = () => resolve()
  ...
  return <div ref={ref} className={`hero-scene phase-${stage}`} data-reveal="0" data-reveal-kind="hero" data-cycling={cycling ? 'true' : 'false'}>
    ...
    <div className="scene-space">
      ...
      <div className="scene-objects">{workflowObjects.map((name, i) => <div className={`scene-object ${stage === i ? 'active' : ''}`} key={name} aria-hidden="true"><img src={`/workflow-objects/${name}.webp`} width="440" height="560" loading="eager" fetchPriority={i === 0 ? 'high' : 'auto'} decoding="async" alt="" draggable={false}/></div>)}</div>
    </div>
