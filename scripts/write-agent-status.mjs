...
const status={schemaVersion:1,updatedAt:now,mode:'AUTONOMOUS_DATA_ONLY',
 ...
 radar:{...radar,discovery,changed:changed.includes('public/radar/index.json')||changed.some(x=>/^public\/radar\/observations\/.+\.json$/.test(x))},
 guard:{coreUiFrozen:true,autonomousWriteScope:['src/content/project-status.generated.ts','public/public-status.json','public/agent-status.json','public/radar/index.json','public/radar/observations/*.json','public/radar/evidence/auto-transaction-*.json']}}
await fs.writeFile('public/agent-status.json',JSON.stringify(status,null,2)+'\n')
console.log('AGENT_STATUS_WRITTEN',status.project.state,status.radar.state,status.radar.discovery.state)
