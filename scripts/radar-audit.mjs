...
const ids=new Set(),checks=new Map(provenance.calls.map(c=>[c.file,c]))
...
for(const item of index.items){
 ...
 assert.equal(doc.cluster,'mainnet-beta');assert.match(doc.signature,/^[1-9A-HJ-NP-Za-km-z]{80,90}$/)
 ...
 assert.equal(raw.transaction.signatures[0],doc.signature);assert.equal(raw.slot,447680207);assert.equal(new Date(raw.blockTime*1000).toISOString(),new Date(doc.observedAt).toISOString())
 assert.equal(checks.get(file).sha256,source.sha256)
 const i=provenance.selected.findIndex(s=>s.signature===doc.signature),status=load('evidence/follow-up-statuses.json').result.value[i]
 assert.equal(status.confirmationStatus,'finalized');assert.deepEqual(status.err,raw.meta.err)
}
// Protect the exact numeric claims, not just valid JSON.
