...
The two Chromium failures in candidate 5d1cc0fd exposed a real minification bug: placing the standard declaration before its WebKit-prefixed equivalent dropped the standard declaration from built CSS. Commit 23e9e302 corrected the order. Candidate 632ceb872 additionally places the canopy at z-index 0 and the header controls at z-index 1; negative canopy stacking is no longer used. The build audit asserts both delivered filter declarations and their foreground stack, not just source text. Source checks, TypeScript and build passed after these changes. Full exact-candidate browser and visual review must still complete before a final pass is claimed.

## Menu-to-page sequencing

The earlier page-entry Web Animation could override hidden-page CSS while the menu was still closing. A dedicated hook now waits for the real menu-settled event instead of estimating the exit duration. Reveals that enter the viewport during closure also wait for that event. Normal menu dismissal does not re-arm completed page content. Finite entry animations and listeners are cancelled/released on route replacement, preference changes or completion. Reopening navigation during page arrival cancels that arrival immediately; a pending animation frame cannot paint through a reopened menu.
...
- Reopen navigation during the destination entrance and verify that the page animation is cancelled.
...
The temporary integration workflow did not force-push after a concurrent documentation update. It was retried against the current branch, preserving that update; the successful commit removed the one-time workflow. No repository permissions or workflow approval settings were changed.
